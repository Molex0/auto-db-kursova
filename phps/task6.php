...
<body>
  ...
  <p>Отримати дані про число ремонтів та їх вартості для автотранспорту певної категорії, окремої марки автотранспорту або вказаної автомашини за вказаний період.</p>
  ...
  if (isset($_GET['submit'])) {
    ...
    if (!empty($auto)) {
      ...
      $sqlSum = "SELECT SUM(r.price) AS total
      FROM repair r
      JOIN transport t ON r.auto = t.id
      WHERE t.id = $auto
        AND r.date BETWEEN '$date1' AND '$date2'";
